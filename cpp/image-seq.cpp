...
unsigned char *applyFilter(unsigned char *arr, unsigned char *result, uint32_t width, uint32_t height, const char *blurOperation, timeMetrics *time);
    ...
    while ((ent_dir_in = readdir(dir_in)) != NULL)
    {
        ...
        {                               //bmp.image = result;
            ...
            if (extension && (strcmp(extension, ".bmp") == 0)) //Check BMP extension
            {
                ...
                if (readBMP(source, &bmp) == 0)
                {
                    //Obtain BMP header and file structure
